...
methodDraw.addExtension("eraser", function (S) {
    ...
    var ChangeElementCommand = svgedit.history.ChangeElementCommand,
        ...
        addToHistory = function (cmd) {
            canv.undoMgr.addCommandToHistory(cmd);
        };
    ...
    return {
        ...
        svgicons: "lib/extensions/eraser-icon.xml",
        ...
        buttons: [{
            ...
            events: {
                'click': function () {
                    ...
                    // Keep the current zoom and leave the zoom control (#zoom_panel) visible so the
                    // student can zoom in to erase small details. (Previously this reset zoom to 100%
                    // and hid the control, because mouseDown mis-mapped coordinates at non-100% zoom;
                    // that mouseDown math is fixed below so erasing now works at any zoom.)
        ...
        mouseDown: function (opts) {
            ...
            if (svgCanvas.getMode() == "eraser") {
               /* $("#strokestyle_div").hide();
                $("#fastcolorpicks").hide();
                $("#fastcolorfillpicks").hide();
                console.log('hiding color pickers from eraser - mousedown');*/
              ...
              */
                ...
                // Capture the current scroll so the view can be restored after the reload below.
                var eraserWorkarea = document.getElementById('workarea');
                var eraserScrollLeft = eraserWorkarea ? eraserWorkarea.scrollLeft : 0;
                var eraserScrollTop = eraserWorkarea ? eraserWorkarea.scrollTop : 0;

                methodDraw.loadFromString(svgCanvas.getSvgString());

                // loadFromString() -> setSvgString() resets the zoom to 100%. Restore the zoom level
                // (and scroll position) the user had so that starting to erase keeps their zoomed-in
                // view instead of snapping back to 100%. This all runs synchronously inside mousedown,
                // so the intermediate 100% state is never painted (no flicker).
                if (lastzoom && lastzoom !== 1) {
                    canv.setZoom(lastzoom);
                    if (methodDraw.updateCanvas) {
                        methodDraw.updateCanvas(true);
                    }
                    if (eraserWorkarea) {
                        eraserWorkarea.scrollLeft = eraserScrollLeft;
                        eraserWorkarea.scrollTop = eraserScrollTop;
                    }
                }
...
////////////////////
                ...
                var e = opts.event;
                // svgcanvas passes start_x/start_y already in (unzoomed) content coordinates, so use
                // them directly. mouseMove receives mouse_x/mouse_y in zoomed coordinates and divides by
                // zoom, ending up in the same content space — the two are now consistent at any zoom.
                ...
                erase_path.el.datum(erase_path.data).attr('d', function (d) {
                    return line(d) + 'Z'
                });
        ...
        mouseMove: function (opts) {
            ...
            if (svgCanvas.getMode() == "eraser") {
             ...
             /*   $("#strokestyle_div").hide();
                $("#fastcolorpicks").hide();
                $("#fastcolorfillpicks").hide();
                console.log('hiding color pickers from eraser - mousemove');*/
                var e = opts.event;
                var zoom = canv.getZoom();
                var evt = opts.event;
        ...
        mouseUp: function (opts) {
            ...
            if (svgCanvas.getMode() == "eraser") {
              /*  $("#strokestyle_div").hide();
                $("#fastcolorpicks").hide();
                $("#fastcolorfillpicks").hide();
                console.log('hiding color pickers from eraser - mouseup');*/
