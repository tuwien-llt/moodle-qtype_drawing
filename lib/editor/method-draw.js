...
*/
                ...
                $.anottationalert = function (msg, cb) {
                    dbox('alert', msg);
                };
                $.anottationdrawing = function (msg, cb) {
                    dbox('alert', msg, null, null, 1, questionid);
                };
            ...
            var selectedChanged = function (window, elems) {
                ...
                if (mode === "select") {
                    ...
                    if (selectedElement) {
                        ...
                        var rows = $('#fastcolorpicks .qtype_drawing_color_divsquare').filter(function () {
                        ...
                        var rowsfill = $('#fastcolorfillpicks .qtype_drawing_color_divsquare').filter(function () {
                            var color = $(this).css("background-color");
                            // console.error(findrgb2hex(color));
                            if (findrgb2hex(color) == fillcol) {
...
//            || elem && elem.tagName == "path" && !multiselected) { // This was added in r1430, but not sure why
            ...
            var changeZoom = function (ctl) {
                // qtype_drawing local fix: input value is now "100%" not "100",
                // so we need parseInt to strip the "%" before division.
                var zoomlevel = parseInt(ctl.value, 10) / 100;
                if (zoomlevel < .001) {
                    ctl.value = .1;
                    return;
                }
...
//console.error("fhd_display_mode",'#' + el_name + '_panel', fhd_display_mode);
      ...
      var changeZoom = function(ctl) {
        // qtype_drawing local fix: input value is now "100%" not "100",
        // so parseInt strips the "%" before division.
        var zoomlevel = parseInt(ctl.value, 10) / 100;
        if(zoomlevel < .001) {
          ctl.value = .1;
          return;
...
*/
            ...
            $('.menu_item').on('mousedown', function (e) {
                blinker(e);
            });
            // qtype_drawing local fix (iPad): blinker schedules
            // `#menu_bar.removeClass('active')` at +220 ms. iOS delays
            // synthetic mouse events (mousedown / mouseup / click) past
            // that, so by the time iOS dispatches them the menu_item is
            // `display:none` and the synthetic mousedown re-targets to
            // the canvas underneath — drawing a stray point with the
            // active tool. preventDefault on touchstart suppresses iOS
            // compat events; we then `.trigger()` the action directly so
            // whichever event the action is bound to (click, mouseup,
            // or mousedown) fires before the menu closes.
            ...
            // qtype_drawing local fix: handle touch via a dedicated touchstart
            // listener with preventDefault. Without this, the browser's compat
            // `mouseover` fires before `mousedown` and pre-adds the `open`
            // class to the tapped menu, defeating any "is this menu already
            // open?" check in the mousedown path. preventDefault on touchstart
            // suppresses the compat mouseover/mousedown for this gesture, so
            // we handle the tap exactly once with the pre-touch state intact.
            var openMenuOnTitle = function ($parent) {
                var $bar = $('#menu_bar');
                var visiblyOpen = $bar.hasClass('active') && $parent.hasClass('open');
                $("#tools_shapelib").hide();
            ...
            $('.menu_title')
                ...
                // qtype_drawing local fix: hover-to-switch removed.
                // Open menus only react to explicit clicks/taps now —
                // mouse moving over a sibling menu_title no longer
                // switches the open menu.
      ...
      */
            ...
            var clickHighlighter = function () {
                if (toolButtonClick('#tool_highlighter')) {
                    svgCanvas.setMode('fhpath');
                    Editor.lastStrokeConfig = {
                        opacity: svgCanvas.getStrokeOpacity(),
                        color: svgCanvas.getColor('stroke'),
                        width: svgCanvas.getStrokeWidth(),
                    };

                    // Highlighter specific settings
                    svgCanvas.setPaintOpacity('stroke', 0.4);
                    svgCanvas.setColor('stroke', window.colorhighlighter);
                    svgCanvas.setStrokeWidth(20);

                    // Update UI inputs to reflect changes (optional but good for UX)
                    $('#group_opacity').val(40);
                    $('#stroke_width').val(20);
...
*/
                ...
                // remove highlight from all others!
                $parentnode.find(".qtype_drawing_color_divsquare").removeClass("qtype_drawing_selectedcolor");
                $parentnode.find(".qtype_drawing_color_pallette").removeClass("qtype_drawing_selectedcolor");
                $parentnode.find("#" + this.id).addClass("qtype_drawing_selectedcolor");
            ...
            var zoomImage = function (multiplier) {
                ...
                $('#zoom').val(parseInt(multiplier * 100) + "% ▼");
            ...
            var clickSaveannotation = function () {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        attemptid: attemptid,
                        attemptcount: attemptcount,
                        annotation: getCurrentDrawingSVG()
                    },
                    cache: false,
...
//        var opacity = (picker == 'stroke' ? $('#stroke_opacity') : $('#fill_opacity'));
          ...
          if(this.type == 'stroke'){
            ...
            // qtype_drawing local fix: clicking the magnifier label only
            // focuses the <select> — natively it does not open the dropdown
            // on desktop. Try showPicker() first (Chrome 99+, Safari 17.4+);
            // otherwise fall through to native <label for="zoom_select">
            // behavior, which on iOS Safari opens the picker.
            $('#zoomLabel').on('click', function (e) {
                var sel = document.getElementById('zoom_select');
                if (!sel) {
                    return;
                }
                if (typeof sel.showPicker === 'function') {
                    try {
                        ...
                        // showPicker can throw in cross-origin iframes; fall through.
                    }
                }
                // No preventDefault: let <label for="zoom_select"> forward the
                // tap to the <select> natively. On iPad this opens the picker.
            });
            ...
            var Actions = function () {
                ...
                var tool_buttons = [
                    ...
                    {sel: '#tool_highlighter', fn: clickHighlighter, evt: 'click'},
            ...
            // Set default zoom
            $('#zoom').val(parseInt(svgCanvas.getZoom() * 100) + "% ▼");
