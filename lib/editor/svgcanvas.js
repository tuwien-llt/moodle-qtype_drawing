...
// Object to contain image data for raster images that were found encodable
    var encodableImages = {},
        ...
        // String indicating the current tool id
...
//          console.log('s',[start_x, start_y]);
            ...
            /*if (current_mode == 'eraser') {
                ...
                console.log("Hiding from svgcanvas - NOT DEFUALT");
            }*/
                    ...
                    if (current_mode === "path") {
                        ...
                        if (curConfig.selectNew) {
                            if (current_mode !== "fhpath" && current_mode !== "eraser") { // Amr 2019, stop selection when drawing.
                                ...
                                if (current_mode !== "fhpath" || current_tool_id !== 'tool_highlighter') {
                                    // If tool is not highlighter, reset selection. this is needed because highlighter is essentially a subset of fhpath
                                    $('#tool_select')[0].click();
                                    $('#tool_fhpath')[0].click();
                                }
...
// The current drawing as raw SVG XML text.
    ...
    window.getHDQuestionID = getHDQuestionID;
...
// name - String with the new mode to change to
    this.setMode = function (name) {
        const $currenttool = $('.tool_button_current');
        if ($currenttool.length > 0) {
            current_tool_id = $currenttool.attr('id');
        } else {
            current_tool_id = name;
        }
        if (methodDraw.lastStrokeConfig) {
            this.setPaintOpacity('stroke', methodDraw.lastStrokeConfig.opacity);
            this.setColor('stroke', methodDraw.lastStrokeConfig.color);
            this.setStrokeWidth(methodDraw.lastStrokeConfig.width);
            methodDraw.lastStrokeConfig = null;
        }
        if (current_tool_id == 'tool_highlighter') {
            $('#fastcolorhighlighterpicks').show();
            $("#strokestyle_div").hide();
            $("#fastcolorpicks").hide();
            $("#fastcolorfillpicks").hide();
        } else {
            $('#fastcolorhighlighterpicks').hide();
            if (name == 'eraser') {
                $("#strokestyle_div").hide();
                $("#fastcolorpicks").hide();
                $("#fastcolorfillpicks").hide();
                ...
                $("#fastcolorpicks").show();
                if (name == 'line' || name == 'fhpath' || name == 'text') {
                    $("#fastcolorfillpicks").hide();
                } else {
                    $("#fastcolorfillpicks").show();
                }
            }
        }
        pathActions.clear();
    ...
    this.setFHDBackground = setFHDBackground = function (im) {
        ...
        if (whichbackground != '') {
            ...
            image.onload = function () {
                // This fires whenever the browser has decoded the image, which can be before
                // method-draw's init() has returned (window.svgCanvas is only assigned
                // afterwards, so calling into it here used to throw and silently skip the
                // background). Wait for the editor instead. The original clear() is gone on
                // purpose: at startup the document is empty anyway, and on a slow-loading
                // background it wiped strokes the student had already drawn. (The "New"
                // button path clears the canvas itself before calling setFHDBackground.)
                let attempts = 0;
                const applyFHDBackground = function () {
                    if (!window.svgCanvas || !window.methodDraw || !window.methodDraw.updateCanvas) {
                        attempts += 1;
                        if (attempts <= 200) {
                            setTimeout(applyFHDBackground, 50);
                        } else if (window.console) {
                            window.console.error('qtype_drawing: editor never became ready, background image not applied.');
                        }
                        return;
                    }
                    window.methodDraw.updateCanvas();
                    window.svgCanvas.setBackground("", whichbackground);
                    window.svgCanvas.setResolution(dimensions[0], dimensions[1]);
                };
                applyFHDBackground();
            }
